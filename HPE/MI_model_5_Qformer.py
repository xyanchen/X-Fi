...
class QFormer(nn.Module):
    def __init__(self, input_dim=512, num_queries=32):
        super(QFormer, self).__init__()
        self.num_queries = num_queries
        self.query_embeddings = nn.Parameter(torch.randn(1, num_queries, input_dim))
    ...
    def forward(self, x):
        ...
        queries = self.query_embeddings.expand(batch_size, -1, -1)  # (batch, num_queries, input_dim)
        
        # Perform attention
        attn_output, _ = self.attention(query=queries, key=x, value=x)  # (batch, num_queries, input_dim)
        
        # Apply layer normalization to stabilize training
        attn_output = self.layer_norm(attn_output)
        # print('QFormer output shape: ', attn_output.shape)
        
        return attn_output  # (batch, 32, 512)

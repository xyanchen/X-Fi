...
class fusion_transformer(nn.Module):
    ...
    # def forward(self, x, modality_list):
        qkv = (q, kv[0], kv[1])
        x = self.mutihead_attention(qkv) + q
...
class fusion_transformer_block(nn.Module):
    ...
    def forward(self, dominant_q, qkv_list, modality_list):
        ...
        for layer in self.transformer_layers:
            ...
            if transformer_layer_idx < len(modality_list):
                if modality_list[transformer_layer_idx] == True:
                    # print(qkv_list[feature_idx_][1:])
                    output = layer(dominant_q, qkv_list[feature_idx_][1:])
                    if feature_idx_ == 0:
                        sum_output = output
                    else:
                        sum_output += output
                ...
                if feature_idx_ == 0:
                    sum_output = layer(dominant_q, qkv_list[feature_idx_][1:])
                else:
                    continue
...
class MI_Transformer(nn.Module):
    def __init__(self, num_modalities, dim, qkv_hidden_expansion, hidden_dim, num_heads, dim_heads, model_depth):
        super().__init__()
        self.qkv_layers = nn.ModuleList([])
        self.transformer_layers = nn.ModuleList([])
        for _ in range(num_modalities):
            self.qkv_layers.append(qvk_projection(dim, qkv_hidden_expansion))
        # for _ in range(model_depth):
        #     self.transformer_layers.append(fusion_transformer_block(dim, hidden_dim, num_heads, dim_heads, num_modalities))
        self.transformer_layers = fusion_transformer_block(dim, hidden_dim, num_heads, dim_heads, num_modalities)
    ...
    def forward(self, x, modality_list):
        ...
        for qkv_layer in self.qkv_layers:
            if modality_list[qkv_layer_idx] == True:
                features[feature_idx], qkv = qkv_layer(features[feature_idx])
                qkv_list.append(qkv)
                feature_idx += 1
                qkv_layer_idx += 1
            else:
                if qkv_layer_idx == len(modality_list) - 1 and feature_idx == 0:
                    features[feature_idx], qkv = qkv_layer(features[feature_idx])
                    qkv_list.append(qkv)
                    qkv_layer_idx += 1
                else:
                    qkv_layer_idx += 1
                continue
        "output qkv_list: [[q_1, k_1, v_1], [q_2, k_2, v_2], ...]"
        "output features: [feature_1, feature_2, ...]"

        r1 = random.randint(0, len(qkv_list)-1)
        dominant_q = qkv_list[r1][0]
        # rest_kv = []
        # for i in range(len(qkv_list)):
        #     if i == r1:
        #         continue
        #     else:
        #         rest_kv.append(qkv_list[i][1,2])
        for i in range(self.depth):
            # print("dominant_q", dominant_q.shape)
            # print("qkv_list", qkv_list)
            # print("modality_list", modality_list)
            dominant_q = self.transformer_layers(dominant_q, qkv_list, modality_list)
        # for transformer_layer in self.transformer_layers:
        #     dominant_q = transformer_layer(dominant_q, qkv_list, modality_list)

        x = self.regression_head(dominant_q)
        return x

    ...
    def forward(self, rgb_feature, depth_feature, mmwave_feature, lidar_feature, csi_feature, lidar_points, modality_list):
        if sum (modality_list) == 0:
            ...
            if modality_list[3] == True:
                ...
                projected_feature += pos_enc_repeat
            else:
                pass
    ...
    def forward(self, feature_embedding, kv_list, modality_list):
        ...
        for layer in self.transformer_layers:
            ...
            if modality_list[transformer_layer_idx] == True:
                ...
                if feature_idx_ == 0:
                    sum_output = new_feature_embedding
                else:
                    sum_output += new_feature_embedding
...
class MI_Transformer(nn.Module):
    ...
    def forward(self, feature, modality_list):
        ...
        features = list(feature.chunk(num_modalities, dim = 1))
        ...
        for kv_layer in self.kv_layers:
            if modality_list[kv_layer_idx] == True:
                kv = kv_layer(features[feature_idx])
                kv_list.append(kv)
                feature_idx += 1
                kv_layer_idx += 1
            else:
        ...
        for i, single_feature in enumerate(features):
            if i == 0:
                feature_embedding = single_feature
            else:
                feature_embedding = torch.add(feature_embedding, single_feature)
        feature_embedding = torch.div(feature_embedding, num_modalities)
        ...
        for i in range(self.depth):
            ...
            feature_embedding = self.transformer_layers(feature_embedding, kv_list, modality_list)
            feature_embedding = self.FFD(feature_embedding)
...
class modality_invariant_model(nn.Module):
    ...
    def forward(self, rgb_data, depth_data, mmwave_data, lidar_data, csi_data, modality_list):
        ...
        projected_features = self.linear_projector(rgb_feature, depth_feature, mmwave_feature, lidar_feature, csi_feature, lidar_data, modality_list)

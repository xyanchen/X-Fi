...
class cross_modal_transformer_block(nn.Module):
    def __init__(self, dim_expansion=2, emb_size = 512, num_heads = 8, dropout=0.3):
        super(cross_modal_transformer_block,self).__init__()
    ...
    def forward(self, feature_embedding):
        feature_embedding_ = self.attention(feature_embedding) + feature_embedding
        out_feature_embedding = self.ffw(feature_embedding_) + feature_embedding_
        return out_feature_embedding
    
class cross_modal_transformer_block_w_pool(nn.Module):
    def __init__(self, dim_expansion=2, emb_size = 512, num_heads = 8, dropout=0.3):
        super(cross_modal_transformer_block_w_pool,self).__init__()
        self.attention = MultiHeadAttention(emb_size, num_heads, dropout)
        self.ffw = FeedForward(emb_size, emb_size*dim_expansion, dropout)
        self.pool = nn.AdaptiveAvgPool2d((32,None))
    
    def forward(self, feature_embedding):
        feature_embedding_ = self.pool(self.attention(feature_embedding)) + self.pool(feature_embedding)
        out_feature_embedding = self.ffw(feature_embedding_) + feature_embedding_
        return out_feature_embedding
...
class cross_modal_transformer(nn.Module):
    def __init__(self, num_feature=32, max_num_modality=4, dim_expansion=2, emb_size = 512, num_heads = 8, dropout=0.3):
        ...
        for _ in range(num_depth-1):
            self.blocks.append(cross_modal_transformer_block(
                dim_expansion=dim_expansion,
                emb_size = emb_size,
                num_heads = num_heads,
                dropout=dropout
            ))
        self.blocks.append(cross_modal_transformer_block_w_pool(
            dim_expansion=dim_expansion,
            emb_size = emb_size,
            num_heads = num_heads,
            dropout=dropout

...
from HAR_lidar_benchmark.pointnet_util import *
...
class feature_extrator(nn.Module):
    def __init__(self):
        ...
        rgb_model.load_state_dict(torch.load('./HAR_RGB_benchmark/rgb_ResNet18/RGB_Resnet18.pt'))
        rgb_extractor = rgb_feature_extractor(rgb_model)
        rgb_extractor.eval()

        depth_model = Depth_ResNet18()
        depth_model.load_state_dict(torch.load('HAR_depth_benchmark/depth_Resnet18.pt'))
        depth_extractor = depth_feature_extractor(depth_model)
        depth_extractor.eval()
        
        mmwave_model = mmwave_PointTransformerReg()
        mmwave_model.load_state_dict(torch.load('HAR_mmwave_benchmark/mmwave_all_random_TD.pt'))
        mmwave_extractor = mmwave_feature_extractor(mmwave_model)
        mmwave_extractor.eval()

        lidar_model = lidar_PointTransformerReg()
        lidar_model.load_state_dict(torch.load('HAR_lidar_benchmark/lidar_all_random.pt'))
...
class MI_Transformer(nn.Module):
    ...
    def forward(self, feature, modality_list):
        ...
        cross_modal_embedding['layer0'] = feature_embedding
...
class modality_invariant_model(nn.Module):
    ...
    def forward(self, rgb_data, depth_data, mmwave_data, lidar_data, modality_list):
        ...
        out, cross_modal_embedding, cross_attention_embedding = self.MIT(projected_features, modality_list)
        # print("output", out)
        # out = out.view(-1, 17, 3)
        return out, feature_list, projected_features, cross_modal_embedding, cross_attention_embedding

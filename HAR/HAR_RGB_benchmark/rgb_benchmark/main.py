...
if __name__ == '__main__':
    ...
    for protocol_setting in setting_list:
        for round_idx in round_list:
            checkpoint_file = os.path.join("/data2/code/mmfi_rgb_save", protocol_setting, round_idx, "epoch_60.bin")
            ...
            model_pos.load_state_dict(checkpoint["model_pos_train"])
            ...
            poses_3d_out = []
            for poses_2d_single in poses_2d_in:
                test_generator = UnchunkedGenerator(None, None, [poses_2d_single],
                ...
                with torch.no_grad():
                    ...
                    for _, _batch, _batch_2d in test_generator.next_epoch():
                        inputs_2d = torch.from_numpy(_batch_2d.astype('float32'))
                        inputs_2d = inputs_2d.cuda() if torch.cuda.is_available() else inputs_2d
                        ...
                        poses_3d_out.append(predicted_3d_pos.squeeze(0).cpu().numpy())
            poses_3d_out = np.array(poses_3d_out)
            poses_3d_in = poses_3d_in.detach().cpu().numpy()
            mpjpe_all, pa_mpjpe_all = [], []
            for i in range(len(poses_2d_in)):
                mpjpe, pa_mpjpe = calulate_error(poses_3d_out[i], poses_3d_in[i])
                mpjpe_all.append(mpjpe)
                pa_mpjpe_all.append(pa_mpjpe)
            mpjpe_all = np.array(mpjpe_all)
            pa_mpjpe_all = np.array(pa_mpjpe_all)
            mpjpe_mean = np.mean(mpjpe_all)
            pa_mpjpe_mean = np.mean(pa_mpjpe_all)
            print("{} {} MPJPE: {}  PA-MPJPE: {}".format(protocol_setting, round_idx, mpjpe_mean * 1000, pa_mpjpe_mean * 1000))
    ...
    #     poses_2d_in = []
    #
    #     if os.path.isfile("./data_cache/" + protocol_setting + "/poses_2d_in.pt") and os.path.isfile("./data_cache/" + protocol_setting + "/poses_3d_in.pt"):
    #         poses_2d_in = torch.load("./data_cache/" + protocol_setting + "/poses_2d_in.pt")
    #         poses_3d_in = torch.load("./data_cache/" + protocol_setting + "/poses_3d_in.pt")
    #     else:
    #         for idx, data in enumerate(train_loader):
    #             poses_2d_in = normalize_screen_coordinates(data['input_rgb'], w=640, h=480)
    #             poses_3d_in = data['output']
    #
    #         os.makedirs("./data_cache/" + protocol_setting, exist_ok=True)
    #         torch.save(poses_2d_in, "./data_cache/" + protocol_setting + "/poses_2d_in.pt")
    #         torch.save(poses_3d_in, "./data_cache/" + protocol_setting + "/poses_3d_in.pt")
    #         print("Saved the data cache into " + protocol_setting)
    #
    #     train_generator = ChunkedGenerator(1024, None, poses_3d_in, poses_2d_in, 1,
    #                                        pad=pad, causal_shift=causal_shift, shuffle=True, augment=True,
    #                                        kps_left=kps_left, kps_right=kps_right, joints_left=joints_left,
    #                                        joints_right=joints_right)
    #

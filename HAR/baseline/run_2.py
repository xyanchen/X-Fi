...
def collate_fn_padd(batch):
    ...
    return input_1, input_2, kpts, exist_list
...
def train(model, train_loader, test_loader, num_epochs, learning_rate, train_criterion, test_criterion, device, modality):
    ...
    for mod in modality:
        name = name + mod + '_'
    name = name + '.pt'
    parameter_dir = './baseline_weights/Dual/' + name
    best_test_mpjpe = 100
...
with open('config_all_2.yaml', 'r') as fd:
...
for i in range(len(config['modality_list'])):
    ...
    for epoch in range(3):
        ...
        for data in val_loader:
            ...
            print(input_1.shape, input_2.shape, kpts.shape)
